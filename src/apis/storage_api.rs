...
/// struct for typed errors of method [`storage_batch_update`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum StorageBatchUpdateError {
    UnknownValue(serde_json::Value),
}
...
pub async fn storage_batch_update(configuration: &configuration::Configuration, table: &str, filter: &str, app_key: &str, request_body: Vec<serde_json::Value>, replace: Option<bool>) -> Result<models::BooleanApiResponse, Error<StorageBatchUpdateError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_table = table;
    let p_filter = filter;
    let p_app_key = app_key;
    ...
    let uri_str = format!("{}/Storage/{appKey}/{table}/batch-update", configuration.base_path, table=crate::apis::urlencode(p_table), appKey=crate::apis::urlencode(p_app_key));
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<StorageBatchUpdateError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}
