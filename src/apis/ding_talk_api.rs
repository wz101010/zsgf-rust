...
/// struct for typed errors of method [`ding_talk_user_info`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum DingTalkUserInfoError {
    UnknownValue(serde_json::Value),
}


/// 根据钉钉用户授权码获取用户的详细资料。
pub async fn ding_talk_user_info(configuration: &configuration::Configuration, app_key: &str, code: Option<&str>) -> Result<models::StringApiResponse, Error<DingTalkUserInfoError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_app_key = app_key;
    let p_code = code;

    let uri_str = format!("{}/DingTalk/{appKey}/UserInfo", configuration.base_path, appKey=crate::apis::urlencode(p_app_key));
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<DingTalkUserInfoError> = serde_json::from_str(&content).ok();
